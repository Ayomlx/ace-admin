...
//        $this->checkToken();
        ...
        $time = time() - $time;
        echojson(array("status" => 1, "info" => "成功备份所选数据库表结构和数据，本次备份共生成了" . ($file_n - 1) . "个SQL文件。耗时：{$time} 秒", "url" => U('SysData/restore')));
    }
    ...
    static private function getRestoreFiles() {
        ...
        if (empty($_GET['sqlPre']))
            die(echojson(array("status" => 0, "info" => "错误的请求")));
//获取sql文件前缀
        $sqlPre = $_GET['sqlPre'];
...
//获取以$sqlPre为前缀的所有sql文件
        ...
        closedir($handle);
        if (count($sqlFiles) == 0)
            die(echojson(array("status" => 0, "info" => "错误的请求，不存在对应的SQL文件")));
...
//检测一行字符串最后有个字符是否是分号，是分号则一条sql语句结束，否则sql还有一部分在下一行中
                if ($tem[$end] == ";") {
                    ...
                    if ($execute > 500) {
                        ...
                        echojson(array("status" => 1, "info" => '如果导入SQL文件卷较大(多)导入时间可能需要几分钟甚至更久，请耐心等待导入完成，导入期间请勿刷新本页，当前导入进度：<font color="red">已经导入' . $imported . '条Sql</font>', "url" => U('SysData/restoreData', array(randCode() => randCode()))));
                        exit;
                    }
                } else {
                    $sql.=$tem;
        ...
        echojson(array("status" => 1, "info" => "导入成功，耗时：{$time} 秒钟"));
    }
    ...
    public function delSqlFiles() {
        if (IS_POST) {
            ...
            if (empty($_POST['sqlFiles']) || count($_POST['sqlFiles']) == 0)
                echojson(array("status" => 0, "info" => "请先选择要删除的文件"));

            $files = $_POST['sqlFiles'];
            foreach ($files as $file) {
                delDirAndFile(DatabaseBackDir . $file);
            }
            echojson(array("status" => 1, "info" => "已删除：" . implode("、", $files), "url" =>U("SysData/restore?" . time())));
        }
    }
    ...
    static private function getSqlFilesGroups() {
        ...
        if (!$_POST['sqlFiles'] || count($_POST['sqlFiles']) == 0)
            die(echojson(array("status" => 0, "info" => "请选择要发送到邮件的sql文件")));

        if (is_email($_POST['to'])) {
            $_SESSION['cacheSendSql']['to'] = $_POST['to'];
        } else {
            die(echojson(array("status" => 0, "info" => "接受SQL文件的邮件地址格式错误")));
        }
        $sqlFiles = array();
...
//发送邮件
                    ...
                    echojson(array("status" => 1, "info" => "如果要发送SQL文件卷较大(多)发送时间可能需要几分钟甚至更久，请耐心等待，发送期间请勿刷新本页。SQL打包成{$sum}个zip包，分{$sum}封邮件发出，<font color=\"red\">当前已经发送完第{$k}封邮件</font>", "url" => U('SysData/sendSql', array(randCode() => randCode()))));
            ...
            die(echojson(array("status" => 1, "info" => "sql文件已发送到你的邮件，请注意查收<br/>耗时：$time 秒"))); //, "url" => __URL__ . "/restore?" . time()
        }
        $this->display();
    }
    ...
    public function zipSql() {
        if (IS_POST) {
            if (!$_POST['sqlFiles'] || count($_POST['sqlFiles']) == 0)
                die(echojson(array("status" => 0, "info" => "请选择要打包的sql文件")));
            $files = $_POST['sqlFiles'];
            $toZip = array();
            foreach ($files as $file) {
                $tem = explode("_", $file);
            ...
            die(echojson(array("status" => 1, "info" => "打包的sql文件成功，本次打包" . count($toZip) . "个zip文件", "url" => U('SysData/zipList'))));
        }
    }
    ...
    function unzipSqlfile() {
        ...
        if ($_SESSION['unzip']) {
            ...
            if (!$_POST['zipFiles'] || count($_POST['zipFiles']) == 0)
                die(evalechojson(array("status" => 0, "info" => "请选择要解压的zip文件")));
        ...
        foreach ($files as $k => $file) {
            ...
            if (count($files) > 1) {
                echojson(array("status" => 1, "info" => "正在解压缩请耐心等待，解压期间请勿刷新本页 <font color=\"red\">当前已经解压完{$file}</font>", "url" => U('SysData/unzipSqlfile', array(randCode() => randCode()))));
        ...
        die(echojson(array("status" => 1, "info" => "已解压完成<br/>耗时：$time 秒"))); //, "url" => U('SysData/restore')
    }
    ...
    public function delZipFiles() {
        if (IS_POST) {
            if (!$_POST['zipFiles'] || count($_POST['zipFiles']) == 0)
                die(echojson(array("status" => 0, "info" => "请选择要删除的zip文件")));
            $files = $_POST['zipFiles'];
            foreach ($files as $file) {
                delDirAndFile(DatabaseBackDir . "Zip/" . $file);
            }
            echojson(array("status" => 1, "info" => "已删除：" . implode("、", $files), "url" =>U("SysData/zipList?" . time())));
        }
    }
    ...
    public function repair() {
        ...
        if (IS_POST) {
            if (empty($_POST['table']) || count($_POST['table']) == 0) {
                die(echojson(array("status" => 0, "info" => "请选择要处理的表")));
            }
            $table = implode(',', $_POST['table']);
            if ($_POST['act'] == 'repair') {
                if ($M->query("REPAIR TABLE {$table} "))
                    die(echojson(array("status" => 1, "info" => "修复表成功", 'url' => U('SysData/repair'))));
            } elseif ($_POST['act'] == 'optimize') {
                if ($M->query("OPTIMIZE TABLE $table"))
                    die(echojson(array("status" => 1, "info" => "优化表成功", 'url' => U('SysData/repair'))));
            }
            die(echojson(array("status" => 0, "info" => "请选择操作")));
        } else {
            $tabs = $M->query('SHOW TABLE STATUS');
